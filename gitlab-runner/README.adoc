...
Operator. Apply CSV via `oc apply -f ClusterServiceVersion.yaml`
...
For GitLab Runners to register themselves with your self-hosted instance of GitLab, you will need to obtain the runner registration token from `Admin -> Overview -> Runners` (for Shared Runners). Apply via `oc apply -f gitlab-runner-secret.yaml`
...
The `Runner.yaml` is used to setup GitLab Runner instances in your OpenShift cluster. Apply via `oc apply -f Runner.yaml`
